...
DATASET_PATH = "../data/celeba64"
# Path to the folder where the pretrained models are saved
CHECKPOINT_PATH = "./saved_models/celeba64"
...
train_set = CelebA(DATASET_PATH, download=True, split='train',
...
test_set = CelebA(DATASET_PATH, download=True, split='valid',
                                        transform=transforms.Compose([
                                        transforms.Lambda(crop),
                                        transforms.Resize(im_size),
                                        transforms.ToTensor(),
...
class CNNModel(nn.Module):
    ...
    def __init__(self, hidden_features=32, out_dim=1, **kwargs):
        ...
        self.cnn_layers = nn.Sequential(
            nn.Conv2d(im_ch, c_hid0, kernel_size=5, stride=2, padding=1),  # [32x32] 
            Swish(),
            nn.Conv2d(c_hid0, c_hid1, kernel_size=3, stride=2, padding=1),  # [16x16]  
            Swish(),
...
def train_model(**kwargs):
    ...
    trainer = pl.Trainer(default_root_dir=os.path.join(CHECKPOINT_PATH, "CelebA64"),
