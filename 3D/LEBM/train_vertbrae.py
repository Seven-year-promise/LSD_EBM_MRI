...
from dataset_vae import CSI_DatasetVAE as CSI_Dataset
##########################################################################################################
## Parameters
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='verbrate', choices=['svhn', 'celeba', 'celeba_crop', 'celeba32_sri', 'celeba64_sri', 'celeba64_sri_crop', 'celeba128', 'verbrate'])
    parser.add_argument('--img_size', default=128, type=int)
    parser.add_argument('--batch_size', default=int(16), type=int)

    parser.add_argument('--nz', type=int, default=100, help='size of the latent z vector')
    parser.add_argument('--nc', default=128)
    ...
    parser.add_argument('--num_channels', default=16, help='Number of Channels for the CNN')
    ...
    parser.add_argument('--n_epochs', type=int, default=100, help='number of epochs to train for') # TODO(nijkamp): set to >100
    ...
    parser.add_argument('--n_fid_samples', type=int, default=100) # TODO(nijkamp): we used 40,000 in short-run inference
...
def get_dataset(args):
    ...
    elif args.dataset == 'verbrate':
        ds_val = CSI_Dataset("./bodies_data_test", mode="test")
        ds_train = CSI_Dataset("./bodies_data_train", mode="train")
...
class _netG(nn.Module):
    def __init__(self, args):
        ...
        self.linear_dec = nn.Linear(args.nz, 16 * 16 * 16 * args.num_channels * 8)

        self.gen = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv3d(args.num_channels*8, args.num_channels*4, 3, padding=1),  # HEEEERE IT WASS IN OUT
            nn.BatchNorm3d(args.num_channels*4),
            f,
            nn.Conv3d(args.num_channels*4, args.num_channels*4, 3, padding=1),  # HAND HERE IN IN
            nn.BatchNorm3d(args.num_channels*4),
            f,

            nn.Upsample(scale_factor=2),
            nn.Conv3d(args.num_channels * 4, args.num_channels * 2, 3, padding=1),  # HEEEERE IT WASS IN OUT
            nn.BatchNorm3d(args.num_channels * 2),
            f,
            nn.Conv3d(args.num_channels * 2, args.num_channels * 2, 3, padding=1),  # HAND HERE IN IN
            nn.BatchNorm3d(args.num_channels * 2),
            f,

            nn.Upsample(scale_factor=2),
            nn.Conv3d(args.num_channels * 2, args.num_channels, 3, padding=1),  # HEEEERE IT WASS IN OUT
            nn.BatchNorm3d(args.num_channels),
            f,
            nn.Conv3d(args.num_channels, args.num_channels, 3, padding=1),  # HAND HERE IN IN
            nn.BatchNorm3d(args.num_channels),
            f,

            nn.Conv3d(args.num_channels, 1, 3, padding=1),
    ...
    def forward(self, z):
        z = z.view(-1, 100)
        z_16 = self.linear_dec(z).view(-1, self.num_channels * 8, 16, 16, 16)
        return self.gen(z_16)
...
class _netWrapper(nn.Module):
    ...
    def sample_langevin_prior_z(self, z, netE, args, verbose=False):
        ...
        for i in range(args.e_l_steps):
            ...
            z_grad_norm = z_grad.view(batch_size, -1).norm(dim=1).mean() # change from args.batch_size to the actual batch_size
    ...
    def sample_langevin_post_z(self, z, x, netG, netE, args, verbose=False):
        ...
        for i in range(args.g_l_steps):
            ...
            z_grad_g_grad_norm = z_grad_g.view(batch_size, -1).norm(dim=1).mean() # change from args.batch_size to the actual batch_size
            z_grad_e_grad_norm = z_grad_e.view(batch_size, -1).norm(dim=1).mean() # change from args.batch_size to the actual batch_size
...
def train(args_job, output_dir_job, output_dir, return_dict):
    ...
    run_name = "Latent_EBM_Vert"
    main_path="./../Vert_data/" + run_name + "/" 

    if not os.path.exists(main_path):
        print('Creating directory at:', main_path)
    ...
    if args.gpu_multi:
        net = torch.nn.DataParallel(_netWrapper(args).to(device), device_ids=[0])
    else:
