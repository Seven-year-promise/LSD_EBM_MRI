...
class VAE_new_small(nn.Module):
    ...
    def __init__(self,num_channels,num_latents):
        super(VAE_new_small, self).__init__()
        ...
        #self.conv_rest_x_16 = self.consecutive_conv(num_channels*4, num_channels*8) 
        self.conv_rest_x_64 = self.consecutive_conv(num_channels, num_channels) 
        self.conv_rest_x_32 = self.consecutive_conv(num_channels, num_channels*2) 
        self.conv_rest_x_16 = self.consecutive_conv(num_channels*2, num_channels*4) 
        
        #self.conv_rest_u_32 = self.consecutive_conv_up(num_channels*8, num_channels*4) 
        #self.conv_rest_u_64 = self.consecutive_conv_up(num_channels*4, num_channels*2) 
        #self.conv_rest_u_128 = self.consecutive_conv_up(num_channels*2, num_channels)
        self.conv_rest_u_32 = self.consecutive_conv_up(num_channels*4, num_channels*2) 
        self.conv_rest_u_64 = self.consecutive_conv_up(num_channels*2, num_channels) 
        self.conv_rest_u_128 = self.consecutive_conv_up(num_channels , num_channels)
         
        self.contract = nn.MaxPool3d(2, stride=2) 
        self.expand = nn.Upsample(scale_factor=2)
        #self.linear_enc=nn.Linear(16*16*16*num_channels*8,num_latents)    
        self.linear_enc=nn.Linear(16*16*16*num_channels*4,num_latents)    
        #self.linear_dec=nn.Linear(num_latents,16*16*16*num_channels*8)
        self.linear_dec=nn.Linear(num_latents,16*16*16*num_channels*4)
    ...
    def encoder(self, x):
        ...
        x_16 = self.conv_rest_x_16(x_16) #rest 128->128->256
        #x_flat=x_16.view(-1,16*16*16*self.num_channels*8) # dimesion becomes 1x... View is used to optimize, since the tensor is not copied but just seen differently
        x_flat=x_16.view(-1,16*16*16*self.num_channels*4) # dimesion becomes 1x... View is used to optimize, since the tensor is not copied but just seen differently
        mean=self.linear_enc(x_flat)
        std=1.e-6+nn.functional.softplus(self.linear_enc(x_flat))
        return mean,std        
    ...
    def decoder(self, x):
        #u_16=self.linear_dec(x).view(-1,self.num_channels*8,16,16,16)
        u_16=self.linear_dec(x).view(-1,self.num_channels*4,16,16,16)
        u_32 = self.expand(u_16)
        u_32 = self.conv_rest_u_32(u_32) #rest 256+128-> 128 -> 128
        u_64 = self.expand(u_32)
        u_64 = self.conv_rest_u_64(u_64)  #rest 128+64-> 64 -> 64
...
class VAE_new_Enc_small(nn.Module):
    def initial_conv(self, in_channels, out_channels):        
        return nn.Sequential(nn.Conv3d(in_channels, int(out_channels/2), 3, padding = 1),
                nn.BatchNorm3d(int(out_channels/2)),
                nn.ReLU(inplace=True),
    ...
    def __init__(self,num_channels,num_latents):
        super(VAE_new_Enc_small, self).__init__()
    ...
    def forward(self,x):
        mean,logvar=self.encoder(x)
        return mean,logvar # note that is the logvar that gets returned


class VAE_new_Dec_small(nn.Module):
    def initial_conv(self, in_channels, out_channels):        
        return nn.Sequential(nn.Conv3d(in_channels, int(out_channels/2), 3, padding = 1),
                nn.BatchNorm3d(int(out_channels/2)),
                nn.ReLU(inplace=True),
    ...
    def __init__(self,num_channels,num_latents):
        super(VAE_new_Dec_small, self).__init__()
    ...
    def forward(self,z):
        return self.decoder(z) # note that is the logvar that gets returned
...
class E_func(nn.Module):
    def __init__(self, latent_dim, inner_dim=1000, out_dim=1):
    ...
    def forward(self, z, t):
        zt = torch.concat([z, t.view(-1,1)], axis=1)
        e = self.ebm(zt)
...
class E_func(nn.Module):
    def __init__(self, latent_dim=100, inner_dim=200, num_layers=1, out_dim=1, batchnorm=False):
        ...
        layers = [apply_sn(nn.Linear(latent_dim+1, inner_dim)), f] + inner_layers + [apply_sn(nn.Linear(inner_dim, out_dim))]
    ...
    def forward(self, z, t):
        zt = torch.cat([z, t.view(-1,1)], axis=1) # TODO: here changed
        e = self.ebm(zt) 
        return  e.view(-1, self.out_dim, 1, 1)
